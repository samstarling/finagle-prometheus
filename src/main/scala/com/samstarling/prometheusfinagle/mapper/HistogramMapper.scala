...
protected class HistogramMapper {
  def apply(rawName: String, snapshot: Snapshot): List[MetricFamilySamples] = {
    ...
    val percentileMetric = new MetricFamilySamples(
      name,
      Collector.Type.GAUGE,
      "No help",
      percentileSamples(name, snapshot))
    val minMetric = new MetricFamilySamples(s"${name}__max",
                                            Collector.Type.GAUGE,
                                            "No help",
                                            sampleWithValue(name, snapshot.max))
    val maxMetric = new MetricFamilySamples(s"${name}__min",
                                            Collector.Type.GAUGE,
                                            "No help",
                                            sampleWithValue(name, snapshot.min))
    val countMetric = new MetricFamilySamples(
      s"${name}__count",
      Collector.Type.GAUGE,
      "No help",
      sampleWithValue(name, snapshot.count))
    val sumMetric = new MetricFamilySamples(s"${name}__sum",
                                            Collector.Type.GAUGE,
                                            "No help",
                                            sampleWithValue(name, snapshot.sum))
